...
static      UIImageView *testImageView_ = nil;
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
  ...
  if (self) {
    ...
    beepSound = -1;
    decoding = NO;
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:CGRectMake(0, 64, self.view.frame.size.width, self.view.frame.size.height - 64)
                                                       cancelEnabled:showCancel 
                                                            oneDMode:oneDMode
                                                         showLicense:shouldShowLicense];
      CGSize codeSize = CGSizeMake(theOverLayView.frame.size.width/3 * 2, theOverLayView.frame.size.width/3 * 2);
      theOverLayView.cropRect = CGRectMake((theOverLayView.bounds.size.width - codeSize.width)/2, (theOverLayView.bounds.size.height - codeSize.height)/2, codeSize.width, codeSize.height);
      testImageView_ = [[UIImageView alloc] initWithFrame:CGRectMake(100, 20, 120, 120)];
//      [self.view addSubview:testImageView_];
//      theOverLayView.backgroundColor = [UIColor redColor];
    ...
    [theOverLayView release];
      ...
      [self.overlayView addSubview:cancel];
  }
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  wasCancelled = NO;
    [self.view bringSubviewToFront:testImageView_];
}
...
#pragma mark -- 修改视频区域
  self.prevLayer.frame = self.overlayView.frame;//self.view.bounds;
...
{ 
...

  {
    ...
    screen.size.height = tmp;
      ...
      NSLog(@"%@",NSStringFromCGRect(self.overlayView.layer.frame));
      NSLog(@"%@",NSStringFromCGRect(self.prevLayer.frame));
      CGRect rect = [self.overlayView.layer convertRect:self.overlayView.cropRect toLayer:self.prevLayer];
